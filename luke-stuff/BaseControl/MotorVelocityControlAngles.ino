...
//struct_robotJointTargets myData;

// Create a structure to hold the readings from each board
//struct_robotJointTargets board1;

// Create an array with all the structures
//struct_robotJointTargets boardsStruct[1] = {board1};

robotJointDataStruct robotJointsData;
pressurePadsDataStruct pressurePadsData;
platformDataStruct platformData;

uint8_t robotMACaddress[] = {0x88, 0x56, 0xA6, 0x57, 0x68, 0x84};//robot
...
/*
  Serial.println();
  const int cur_millis = millis();
  const int delta = cur_millis - timeSinceLastPacket;
  totalLatency += delta;

  timeSinceLastPacket = cur_millis;
  Serial.printf("Time since last packet: %d", delta);
  Serial.println();
...
*/
const uint8_t* mac_addr = messageInfo->src_addr;
if(mac_addr[0] == robotMACaddress[0] && mac_addr[1] == robotMACaddress[1] && mac_addr[2] == robotMACaddress[2] && mac_addr[3] == robotMACaddress[3]
  && mac_addr[4] == robotMACaddress[4] && mac_addr[5] == robotMACaddress[5]) {
  memcpy(&platformData, incomingData, sizeof(platformData));
}
...
void calculateVelocities(float theta, float scalar, float omega, int &m1, int &m2, int &m3){
  float vx = cos(theta)*scalar;
  float vy = sin(theta)*scalar;

  float w1 = 0.866*vx + 0.5*vy + omega*ROBOT_RADIUS;
  float w2 = -vy + omega*ROBOT_RADIUS;
  float w3 = -0.866*vx + 0.5*vy + omega*ROBOT_RADIUS;
...
void move(SMS_STS st, float theta, float scalar, float omega, int &m1, int &m2, int &m3){
  calculateVelocities(theta, scalar, omega, m1, m2, m3);
...
void moveTo(SMS_STS &st, float targetX, float targetY, int &m1, int &m2, int &m3){
  ...
  while(true){
    ...
    move(st, moveTheta, scalar, 0, m1, m2, m3);
...
void trackUser(SMS_STS &st, float targetX, float targetY, int &m1, int &m2, int &m3){
  ...
  static float prevErrorTheta = 0;
  updatePos(st);
  float errorX = targetX - x;
  float errorY = targetY - y;
  float errorTheta = 0 - theta;
  ...
  float dErrorTheta = errorTheta - prevErrorTheta;
  ...
  float omega = KP*errorTheta + KD*dErrorTheta;
  ...
  move(st, moveTheta, speed, omega, m1, m2, m3);
}
...
void loop() {
  ...
  // for(int i=0; i<50; i++){
  //   trackUser(st, 1, 0.5, m1, m2, m3);
  //   delay(10);
  // }
  // for(int i=0; i<50; i++){
  //   trackUser(st, 0, -0.5, m1, m2, m3);
  //   delay(10);
  // }

  trackUser(st, platformData.z/1000, platformData.x/1000, m1, m2, m3);
  Serial.print("Target X: ");
  Serial.print(platformData.x/1000, 6);
  Serial.print(" Target Z: ");
  Serial.println(platformData.z/1000, 6);
  //delay(10);
